...
- All scholarly claims in the primary article must be supported by at least three scholarly sources (from your Zotero + public literature). If a claim cannot be supported with ≥3 sources, mark it as **authorвЂ‘asserted** and flag for empirical validation in Terra.
...
## Immediate plan: StepвЂ‘byвЂ‘step (1 → 5) — do all in parallel, integrated & iterative
...
- Run semantic detoxification on all humanвЂ‘facing strings and generated content using Terra Detox protocol (L0–L3). Save original and cleaned versions with diff metadata.
...
- Package everything in a single `terra-release-v1.0.zip` with a clear `LICENSE` (you choose — recommend CC BYвЂ‘SA for docs and MIT for code), contributor list, and a `HOWTO` for reviewers and collaborators.
...
3. Create the topвЂ‘level Research Assistant brief (this document — done) and produce the initial structure for the monorepo (folders and README placeholders).
